...
public final class FollowManagerDatastoreTest {
  ...
  public void testFollow() {
    // test follow no one
    assertEquals(new ArrayList<>(), manager.getFollowedUsers(USER_ID_A));
    ...
    assertEquals(list_users, manager.getFollowedUsers(USER_ID_A));
    ...
    assertEquals(list_restaurant, manager.getFollowedRestaurants(USER_ID_B));

    // test unfollow restaurant
    manager.unfollowRestaurant(USER_ID_B, RESTAURANT_ID);
    assertEquals(new ArrayList<>(), manager.getFollowedUsers(USER_ID_B));

    // test follow tag
    manager.followTag(USER_ID_B, TAG_ID);
    List<Long> list_tag = new ArrayList<>();
    list_tag.add(TAG_ID);
    assertEquals(list_tag, manager.getFollowedTags(USER_ID_B));
